...
<div class="requirementCompositions index large-9 medium-8 columns content">
    <h3><?= __('Requirement Compositions') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('composition_dosage_reference') ?></th>
                <th scope="col"><?= $this->Paginator->sort('composition_pharma_designation') ?></th>
                <th scope="col"><?= $this->Paginator->sort('requirement_id') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($requirementCompositions as $requirementComposition): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $requirementComposition->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $requirementComposition->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $requirementComposition->id], ['confirm' => __('Are you sure you want to delete # {0}?', $requirementComposition->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
