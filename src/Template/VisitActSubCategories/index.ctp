...
<div class="visitActSubCategories index large-9 medium-8 columns content">
    <h3><?= __('Visit Act Sub Categories') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('visit_act_category_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('label_sub_category') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($visitActSubCategories as $visitActSubCategory): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $visitActSubCategory->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $visitActSubCategory->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $visitActSubCategory->id], ['confirm' => __('Are you sure you want to delete # {0}?', $visitActSubCategory->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
